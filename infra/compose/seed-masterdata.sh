...
CONN="Host=localhost;Port=5432;Database=hbmp;Username=${POSTGRES_USER};Password=${POSTGRES_PASSWORD}"
...
echo "==> Row counts:"
PGPASSWORD="${POSTGRES_PASSWORD}" psql -h localhost -U "${POSTGRES_USER}" -d hbmp -c \
