...
| 54 | The Check That Checked Nobody — an eligibility screen that searched on any fragment of a name and ran the check against `hits[0]`, so the plan, remaining cap and visit verdict on the card belonged to whichever member the database returned first; replaced by a verified lookup that takes an identifier the beneficiary presented plus part of their name, and refuses without naming anybody (2026-08-21) | [54-the-check-that-checked-nobody.md](54-the-check-that-checked-nobody.md) | D |
