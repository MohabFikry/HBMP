...
| 22 | [phase-22-completion.md](phase-22-completion.md) | **Audit-R3 remediation:** the gate-per-finding pass that closed the R1/R2/R3 audit trail — event symmetry, coverage floors, the freshness watchdog, and the CI-truth work the three audits kept re-finding | 26, 34 | — | R3 |

<!-- There is no phase 23. It was never written: the R3 remediation work planned for it was folded into 22,
     and 24 was numbered before anybody noticed the gap. Recorded here rather than left as a hole, because a
     missing number in a sequence reads as a lost document and somebody goes looking for it — the 2026-08-09
     audit did, and filed it as a phantom dependency. -->

| 24 | [phase-24-coverage-ci-truth-and-hygiene.md](phase-24-coverage-ci-truth-and-hygiene.md) | **Coverage, CI truth & hygiene:** made the gates say what they measure — per-module floors that ratchet, the empty-`<main>` race that had half the a11y sweep auditing nothing, and the skipped-test census | 26, 34 | — | R3 |
...
| 30 | [phase-30-order-amendment.md](phase-30-order-amendment.md) | **Amend & cancel signed orders:** a prescriber may cancel or amend a **signed** prescription / lab / radiology / OP procedure while **unconsumed** — **supersede, never mutate**; **line-level** scope so the consumed portion stays immutable; the "not yet dispensed" check is **one atomic guarded transition** with a conflict message naming what happened; chronic **duration & frequency** edits that leave dispensed windows byte-identical and still sum exactly; **in-scope vs out-of-scope** amendments decide whether authorisation is re-required; propagation **updates the provider's queue**, not just a notification; coded mandatory reasons · **notes on every order kind** reusing the doc-38 append-only signed model, three visibility classes (an external provider never sees `Internal`), sensitivity inherited, and **a note is never an amendment** · + `OccupationalTherapy` and `SpeechTherapy` seeded as session-based procedure types | **46**, 23, 45, 19 | — | independent |
