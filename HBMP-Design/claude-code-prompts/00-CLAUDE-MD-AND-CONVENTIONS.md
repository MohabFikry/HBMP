...
> Copy everything below the line into a `CLAUDE.md` at the repository root. Claude Code loads it automatically each session, so these rules apply to every phase prompt without repetition. Adjust names/versions to match final decisions in [../HBMP-Design/0A-DESIGN-FOUNDATIONS.md](../HBMP-Design/0A-DESIGN-FOUNDATIONS.md).
...
This project ships 20 custom domain skills in `HBMP-Design/claude-code-skills/` (index: `00-SKILLS-INDEX.md`). Install them under `.claude/skills/`. They encode Mersal's benefit rules, state machines, minimum-necessary zoning, and brand system — knowledge generic skills lack.

- **At the start of every phase session, activate the skills listed for that phase** in `claude-code-skills/00-SKILLS-INDEX.md` (Phase → skills mapping). Each phase prompt file also names its skills.
