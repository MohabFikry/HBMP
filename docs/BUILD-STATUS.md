...
| 16 | Audit Remediation | 16.8 CI & test integrity (skippable tests, PG service, coverage, ESLint, OpenAPI) (H7) | ◐ | **skippable-tests** (`7b9b665`): env-gated DB tests → `[SkippableFact]`+`Skip.If`. **PG-in-CI + coverage DONE**: new `.github/workflows/backend-ci.yml` (postgres:16 service, warnings-as-errors build) + `tools/ci/apply-migrations.sh` (provisions hbmp_app + applies all 77 migrations to a clean DB) + `print-test-db-env.sh` (wires the 11 integration + 13 RLS-pair + events `*_TEST_DB*` vars so DB/RLS suites RUN not skip) + `coverage-gate.sh` (domain line-rate floor, `COVERAGE_MIN_DOMAIN` default 55, measured 58.1%, target 80%). **Verified locally** against a fresh scratch DB on PG:55432 — bootstrap → 18 schemas/155 tables, **full solution 0-fail 0-skip** (every service incl. masterdata-dependent claims/orders/pharmacy self-seed; RLS runs under hbmp_app). Remaining: ESLint gate (no eslint infra yet — tsc `lint`+axe already gate FE), OpenAPI check (Swashbuckle is runtime-only — needs a spec-generation step) |
