...
| 16 | Audit Remediation | 16.7 Live frontend wiring (scopes/roles/env/errors/i18n) (H6/H10) | ◐ | identity slice: live KC roles/users/scopes (call_center + claims_officer) + fail-closed role map (no-portal page) + provision-identity.sh. **errors DONE**: `http.ts` parses RFC 7807 problem+json → `ApiError.problem`/`.reason`; `AsyncSection` shows kind-specific headline + server `detail` (5 http.test.ts). **env DONE**: `apps/web/.env.example` documents VITE_* (LIVE/API_BASE/OIDC). **idempotency** verified already-done (HttpApiClient sends Idempotency-Key on every mutation). Remaining: i18n hardcoded-string leak audit |
| 16 | Audit Remediation | 16.8 CI & test integrity (skippable tests, PG service, coverage, ESLint, OpenAPI) (H7) | ◐ | skippable-tests slice DONE (`7b9b665`): env-gated DB tests → `[SkippableFact]`+`Skip.If` (Xunit.SkippableFact) so unconfigured tests report *skipped* not falsely-green; solution 0-warning, suite 0-fail (164 skipped when *_TEST_DB unset). Remaining: PG service in CI, coverage gate, ESLint gate, OpenAPI check |
