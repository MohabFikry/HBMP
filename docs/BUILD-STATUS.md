...
| 16 | Audit Remediation | 16.9 Small items + ADRs 0010–0014 | ◐ | **ADRs 0010–0014 all exist** (0010 consume-executor, 0011 hbmp_app-RLS, 0012 sensitive-grants, 0013 durable-outbox, 0014 phase-14-retrofit) — ADR half DONE. `hello` fully retired (out of sln/compose/Kong; `up.sh` stale build+endpoint refs removed; source retained for reference per compose comment). CallCentre i18n enum leak fixed (REASONS/CANCEL_REASONS → bilingual labels, enum stays the option value). **problem+json DONE**: all ~104 bare `Results.NotFound()` → `Results.Problem` (RFC 7807, 41 files/14 svcs; suite 22/22 green). **migrations-copy-to-output DONE**: added the copy rule to the 8 Infra projects that missed it (all 18 consistent). **Remaining polish tail** (low-priority, independent): Idempotency-Key on admin/document/finance/reporting mutations; anonymous error objects (BadRequest/Conflict) → problem+json; ~14 `*Gate.cs` dedup → libs/authz; eligibility consumer health-check; FE: If-Match/ETag/412 flow, RestrictedResultCard design tokens, finance export date window, CallCentre history error handling, reschedule impl, web Dockerfile, more screen tests |
