...
| 16 | Audit Remediation | 16.9 Small items + ADRs 0010–0014 | ◐ | **ADRs 0010–0014 all exist** (0010 consume-executor, 0011 hbmp_app-RLS, 0012 sensitive-grants, 0013 durable-outbox, 0014 phase-14-retrofit) — ADR half DONE. `hello` fully retired (out of sln/compose/Kong; `up.sh` stale build+endpoint refs removed; source retained for reference per compose comment). CallCentre i18n enum leak fixed (REASONS/CANCEL_REASONS → bilingual labels, enum stays the option value). **Remaining polish tail** (low-priority, independent): ~104 bare `Results.NotFound()` → problem+json across 14 svcs; Idempotency-Key on admin/document/finance/reporting mutations; ~14 `*Gate.cs` dedup → libs/authz; migrations-copy-to-output (8 svcs); eligibility consumer health-check; FE: If-Match/ETag/412 flow, RestrictedResultCard design tokens, finance export date window, CallCentre history error handling, reschedule impl, web Dockerfile, more screen tests |
