...
| 18.D2 | U5 | App-bar global search is a dead field bound to `/` | ⚠️ **REOPENED 2026-07-27 by product decision** — the original `SearchField` input is restored in the app bar (visible affordance wanted while record search is designed). It still accepts typing with nowhere to send it, and `/` focuses it. The 18.F2 command palette (⌘K) remains and works. **To close:** wire the field to a permission-scoped endpoint with min-necessary field rules + a PHI-read audit, or route it into the palette. |
