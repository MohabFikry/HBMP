...
| **The validity artefact** `ValidityArtefact.ImagingOrder` | A persisted *config* vocabulary keyed on `validity.imaging-order.days`, with a configured row per tenant. Same trade as the benefit category: renaming it would rewrite live configuration to chase a label. **Added after the fact** — this row was missing from the original table, and the omission mattered, because the screen's English LABEL for it had also been left saying "Imaging orders" while its Arabic already read الأشعة. The label is now `Radiology orders`; the key stays. `ValidityPolicyAdmin`'s `ARTEFACT_LABEL` is the seam, and `validity-admin.test.tsx` pins it. |
