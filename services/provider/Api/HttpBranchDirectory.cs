...
/// <summary>25.2 — provider-service's copy of the admin-service branch-directory client. The same shape as
/// emr's, orders' and policy's, and duplicated for the same reason they are: <c>libs/authz</c> owns the
/// <see cref="IBranchDirectory"/> SEAM and deliberately takes no dependency on HttpClient or IMemoryCache, so
/// each service brings its own transport. If a fifth copy appears, that is the point to lift it.
///
