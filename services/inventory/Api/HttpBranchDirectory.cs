...
/// <summary>25.6 — inventory-service's copy of the admin-service branch-directory client. The same shape as
/// emr's, orders', policy's and provider's, and duplicated for the same reason they are: <c>libs/authz</c> owns the
/// <see cref="IBranchDirectory"/> SEAM and deliberately takes no dependency on HttpClient or IMemoryCache, so
/// each service brings its own transport. This is the FIFTH copy — the point at which lifting it into a
/// shared package stops being premature. Deliberately not done inside this phase: it touches five services'
/// dependency graphs, which is its own reviewable change and not one to smuggle into an inventory build.
///
/// Reads a caller's permitted branch set from admin-service (<c>GET /api/v1/me/branches</c>),
