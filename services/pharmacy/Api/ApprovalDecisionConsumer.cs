...
public sealed class ApprovalDecisionConsumerOptions
{
    ...
    /// <summary>pharmacy-service's OWN decision queue — see <see cref="ApprovalDecisionFeed"/> for why orders
    /// and pharmacy each get their own copy rather than sharing <c>approvals.events</c>.</summary>
    public string DecisionQueue { get; set; } = ApprovalDecisionFeed.PharmacyQueue;
...
/// <summary>
/// Releases (or terminally refuses) the prescription an authorization decision was about.
/// </summary>
/// <remarks>
/// <para><b>Why this service is a consumer at all.</b> pharmacy-service published <c>RxSubmitted</c> and
/// nothing came back: no service consumed <c>approvals.events</c>, so <c>Submitted → Approved</c> was
/// declared in <c>PrescriptionWorkflow</c> and executed only by the auto-route at creation. A gated script
/// could therefore never reach a status <c>IsDispensable</c> admits. This is the code that moves it.</para>
/// <para><b>Why asynchronous rather than a callback from approvals.</b> The reviewer must be able to reject a
/// request while pharmacy-service is restarting; a synchronous release would make one service's availability
/// a precondition for the other's decisions. The one place the platform DOES call synchronously — a validity
    ...
    ILogger<ApprovalDecisionConsumer> logger) : BackgroundService
{
    ...
    protected override Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        try
        {
            ...
            _connection = factory.CreateConnection("pharmacy-service-approvals");
            ...
            logger.LogInformation("pharmacy-service consuming authorization decisions from {Queue}", opt.DecisionQueue);
        }
        catch (Exception ex)
        {
            // Broker unavailable (unit/dev without RabbitMQ): serve the API rather than crash the host. Gated
            // scripts stay Submitted until the broker returns; nothing is lost, because the decision is
            // durable in approvals' outbox until relayed and acked here.
            logger.LogWarning(ex, "pharmacy approval-decision consumer could not connect; approved prescriptions are not becoming dispensable");
        }
    ...
    private async Task OnReceivedAsync(BasicDeliverEventArgs ea, CancellationToken ct)
    {
        ...
        try
        {
            ...
            {
                // Dead-letter, do not requeue: a decision we cannot attribute to a tenant or a prescription
                // will not become attributable by being delivered again, and applying it under a guessed
                // tenant would make somebody else's script dispensable.
            ...
            {
                var result = await sp.GetRequiredService<PrescriptionApprovalApplier>().ApplyAsync(msg, ct);

                db.ProcessedEvents.Add(new ProcessedEvent { EventId = eventId, ProcessedAt = clock.GetUtcNow() });
                await db.SaveChangesAsync(ct);

                // Audited when the prescription actually moved. A decision about an investigation order
                // arrives here too (both queues get every decision) and produces nothing — an audit row per
                // ignored message would bury the state changes among them.
                if (result.Outcome is ApprovalApplyOutcome.Released or ApprovalApplyOutcome.Rejected)
                {
                    await sp.GetRequiredService<IAuditClient>().EmitAsync(new AuditEventDraft
                    {
                        ...
                        EntityId = result.PrescriptionId!.Value.ToString(),
                        Action = AuditAction.StateChange,
                        TenantId = msg.TenantId,
                        BeforeState = result.Detail,
                        AfterState = result.Outcome == ApprovalApplyOutcome.Released ? "Approved" : "Rejected",
                ...
                if (result.Outcome == ApprovalApplyOutcome.NotWaiting)
                    logger.LogWarning(
                        "authorization {AuthNo} decided, but prescription {RxNo} was not waiting on it ({Detail})",
                        msg.AuthNo, result.RxNo, result.Detail);
            }
        ...
        catch (Exception ex)
        {
            ...
            if (attempt > options.Value.MaxRedeliveries)
            {
                ...
                logger.LogCritical(ex,
                    ...
                    "requeued again. AN APPROVED PRESCRIPTION IS STILL NOT DISPENSABLE — replay it from " +
