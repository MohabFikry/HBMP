...
> Phase 7.1 (this slice) delivers the service foundation + reviewer worklist + the clinical **review view**.
> Decisions with downstream gate release (7.2) and break-glass + SLA/TAT (7.3) follow.
...
- `Infrastructure/Migrations/0001_approvals.sql` — schema, `auth_seq`, both tables, the append-only trigger,
  `processed_request` idempotency ledger. Applied to host PG (:55432).
